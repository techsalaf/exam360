<div class="modal fade" id="editCouponModal{{ $coupon->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST" class="modal-content">
            @csrf
            @method('PUT')
            <div class="modal-header">
                <h5 class="modal-title fw-bold">{{ __('coupons.modal_edit_title') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-start">
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted">{{ __('coupons.label_code') }}</label>
                    <input type="text" name="code" class="form-control text-uppercase fw-bold" value="{{ $coupon->code }}" required>
                </div>
                <div class="row g-3 mb-3">
                    <div class="col-6">
                        ...
                        <select name="type" class="form-select">
                            ...
                            <option value="fixed" {{ $coupon->type == 'fixed' ? 'selected' : '' }}>{{ __('coupons.opt_fixed') }} ({{ $currencySymbol }})</option>
                            <option value="percentage" {{ $coupon->type == 'percentage' ? 'selected' : '' }}>{{ __('coupons.opt_percentage') }} (%)</option>
                        </select>
                    </div>
                    <div class="col-6">
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6">
                        ...
                        <small class="text-muted d-block mt-1 help-text-small">{{ __('coupons.help_unlimited') }}</small>
                    </div>
                    <div class="col-6">
                        <label class="form-label small fw-bold text-muted">{{ __('coupons.label_min_purchase') }}</label>
                        <div class="input-group">
                            <span class="input-group-text">{{ $currencySymbol }}</span>
                            <input type="number" name="min_purchase" step="0.01" class="form-control" value="{{ $coupon->min_purchase }}">
                        </div>
                    </div>
                </div>
                <div class="mb-3">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-green-pill">{{ __('coupons.btn_save') }}</button>
            </div>
        </form>
    </div>
</div>